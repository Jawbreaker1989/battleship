...
public class BoardPanel extends JPanel {
    ...
    private static final int CELL_SIZE = 45; // Aumentado de 40 a 45 para celdas más grandes
    ...
    private void tryPlaceShip(int x, int y) {
        ...
        // Construir posiciones inicio/fin para el request remoto
        Position start = new Position(x, y);
        Position end = isHorizontal ? new Position(x + shipSize - 1, y) : new Position(x, y + shipSize - 1);

        GameController controller = parentWindow.getController();
        if (controller == null || !controller.isConnected()) {
            parentWindow.showMessage("⚠️ Aún no conectado al servidor");
            return;
        }

        boolean remoteOk = controller.placeShip(start, end);
        if (!remoteOk) {
            parentWindow.showMessage("❌ Servidor rechazó el barco (verifica solapamiento o inventario)");
            return;
        }

        // Solo reflejar localmente si el servidor aceptó
    ...
    public void clearBoard() {
        ...
        for (int i = 0; i < BOARD_SIZE; i++) {
            for (int j = 0; j < BOARD_SIZE; j++) {
                board[i][j] = isMyBoard ? CellState.WATER : CellState.UNKNOWN;
            }
        }
        
        // Resetear variables de colocación de barcos
        currentShipIndex = 0;
        shipsPlaced = 0;
        attackMode = false;
